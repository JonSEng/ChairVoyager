...
#define WLAN_SSID       "Haley Rowland's iPhone"           // cannot be longer than 32 characters!
...
void loop(void)
{
  ...
  Serial.println("raw light: " + light);
  light.toCharArray(fieldData, 4);
...
void postData()
{
  ...
  if (www.connected()) {
    ...
    www.fastrprint(F("GET /input/"));
    www.fastrprint(F(publicKey));
    www.fastrprint(F("?private_key=?"));
    www.fastrprint(F(privateKey));
    Serial.print("Printing fieldNames:");
    Serial.println(fieldNames);
    Serial.print("Printing fieldData:");
    Serial.println(fieldData);
    ...
    www.fastrprint(F("\r\n"));
    www.println();
    Serial.println("postData finished");
  } else {
    Serial.println(F("Connection failed"));    
    return;
  }
